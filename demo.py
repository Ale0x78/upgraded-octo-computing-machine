...
class Graph:
    def __init__(self, maxLayers=10, filename=None) -> None:
        ...
        if filename:
            ...
            self.goals.add(State(input("Please enter your goal: "), [''] ))
            actions = []
            state = set()
            action_mutexes = []
            state_mutexes = []
            ...
            for item in starting_items:
                state.add( State(item[0], item[1]) )
                self.all_actions.append(Action('NoOp' + item[0], [State(item[0], item[1])], [State(item[0], item[1])]))
